...
## 9. Round 4 — production-hardening pass on `execute_async` cancellation

**Trigger:** user-directed re-review of §8.4 finding #3 (the nbclient cancel/dead-kernel race), explicitly asking for the *root cause* fixed, not the test loosened to tolerate it — "do not default to local fixes, isolated prompt tweaks, or short-term workarounds."

**Reassessment.** Round 3's fix (explicit kernel cleanup before re-raising `CancelledError`) was necessary but incomplete: it closed the *process-leak* bug but left a second, distinct bug unaddressed — this module's own public contract for `execute_async()` was non-deterministic under cancellation, because classification was based on `isinstance(exc, asyncio.CancelledError)` alone, and nbclient's own internal race (§8.4 #3) can hand back `DeadKernelError` instead of `CancelledError` for a cancellation that was genuinely requested. Round 3's test change (assert the invariant that holds either way) correctly described the *symptom* but left the *defect* — an unreliable `except asyncio.CancelledError` for any real caller — unfixed in the shipped code.

**Root cause, precisely:** nbclient's `async_execute_cell` contains `except asyncio.CancelledError: raise DeadKernelError("Kernel died") from None` around `await self.task_poll_for_reply`, on the false assumption that only nbclient's own internal watchdog task can trigger it. An externally cancelled enclosing task can deliver `CancelledError` to that exact `await`, which nbclient then silently converts. Which `await` is active at the moment of cancellation is a function of event-loop scheduling jitter across several concurrently running internal tasks — genuine, unavoidable non-determinism at that layer, not fixable by retrying or waiting longer.

**Fix (structural, not a workaround):** classify "was this task's own cancellation requested" using `asyncio.current_task().cancelling()` (Python 3.11+, this project's floor) in addition to `isinstance(exc, CancelledError)`, so the classification no longer depends on which exception nbclient's race happens to produce. Extracted as a pure function, `_is_requested_cancellation(exc, cancelling_count) -> bool`, so the decision logic is unit-testable in milliseconds independent of any real kernel (`tests/unit/test_jupyter_execute_cancellation.py`, 5 tests). `execute_async()` now: if a cancellation was requested (by either signal), does the existing best-effort kernel cleanup, then re-raises the real `CancelledError` if that's what nbclient gave back, or synthesizes one (`raise asyncio.CancelledError() from exc`) if nbclient swallowed it — keeping the discarded exception visible via `__cause__` instead of losing it the way nbclient's own `from None` does.

**What this does and does not fix:** it does not patch nbclient's own bug (impossible from outside). It guarantees libipynb's own contract holds regardless: `except asyncio.CancelledError:` around a cancelled `execute_async()` call is now reliable. If nbclient changes this internal behavior in a future release, this code stays correct (the extra branch simply never fires) but the documented finding becomes stale — noted explicitly in the module's own docstring rather than left to be silently rediscovered.

**Validation:** the real-kernel regression test (`test_execute_async_cancellation_propagates_and_cleans_up_the_kernel`) was rewritten to run 5 cancellation trials per invocation and assert deterministic `CancelledError` delivery plus zero leaked kernel processes on every trial — a single pass proves little about a race that only manifested some of the time before the fix. Run 4 times in a row after the fix (20 total trials): **20/20 deterministic `CancelledError`, 0 leaked processes.** Before the fix, roughly half of single-trial runs surfaced the race. Full regression suite, `ruff`, `mypy` all reproduced green after this change — see §9.1.

### 9.1 Final regression (reproduced live, this round)

```
pytest tests/ -q                              -> 879 passed, 4 skipped, 282.71s (0:04:42)
                                                  (up from 874/4 at the end of Round 3; +5 net new
                                                  tests -- 5 pure classification unit tests, the
                                                  real-kernel test itself was rewritten in place
                                                  rather than added -- zero regressions)
ruff check .                                  -> All checks passed!
ruff format --check .                         -> 137 files already formatted
mypy src/libipynb                             -> Success: no issues found in 40 source files
```

**Files changed this round:** `src/libipynb/adapters/jupyter_execute.py` (cancellation classification + docstring), `tests/integration/test_obligation_jupyter_execution_adapter.py` (rewritten cancellation test), new `tests/unit/test_jupyter_execute_cancellation.py`.
