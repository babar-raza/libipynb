...
**Status:** Rounds 1–3 executed. Round 1 (2026-08-13): 9 of 17 taskcards (`P1`, `P2`, `P3a`, `P3b`, `P3c`, `P6`, `P7`, `P8`, `P9`) implemented and regression-verified. Round 2 (2026-08-13): Gate G8 oracle comparisons for `P2`/`P3c` against real installed reference tools. **Round 3 (2026-08-17):** Gate G6 sign-off recorded (§7) and `P4a-1`/`P4b`/`P4c` (real Jupyter-kernel-protocol execution engine, rich output write-back, CLI `execute` subcommand) implemented and regression-verified, including a real Gate G8 oracle comparison against `nbconvert --execute` — see [full-parity-execution-evidence.md](full-parity-execution-evidence.md) §8. Final regression: 874 passed, 4 skipped, `ruff`/`mypy` clean. `P4a-2`/`P5a`/`P5b`/`P5c` remain `not_attempted`/deferred (genuinely out of Round 3's mission scope, not silently dropped — see §8.9); `P3d`/`P5c` remain explicitly deferred stretch items.
**Date:** 2026-08-13 (drafted and hardened; Rounds 1–2 executed same day), Round 3 executed 2026-08-17.
**Author:** produced by an assistant session, from a fresh forensic pass; Round 1 execution by the same session under an explicit autonomous-execution mandate. Round 3 executed by a separate assistant session under an explicit, written maintainer mission specification (verified as the maintainer, Babar Raza — see §7's sign-off log).
...
| `LIBIPYNB-P4a-1` | nbconvert/nbclient parity: real kernel-protocol execution engine, Python-only pilot | absorbs/completes `V4` (reframed — see §9) | `completed_verified` (2026-08-17) | P1 | Execution Engine | P7, P8, P9, **Gate G6 sign-off** |
| `LIBIPYNB-P4a-2` | nbconvert/nbclient parity: multi-language kernel support | extends P4a-1 | `not_attempted`, explicitly deferrable (architecture does not block it — see P4a-1's closeout note) | P2 | Execution Engine | P4a-1 proven in production use |
| `LIBIPYNB-P4b` | Rich output write-back into notebook `outputs` | prerequisite for P5 | `completed_verified` (2026-08-17) | P1 | Execution Engine | P4a-1 |
| `LIBIPYNB-P4c` | CLI `execute` subcommand | absorbs `V6` (execute half) | `completed_verified` (2026-08-17) | P2 | Execution Engine | P4a-1, P4b |
...
**Status:** `completed_verified` (2026-08-17, Gate G6 sign-off recorded §7; Gate G1/G2/G8 evidence in `full-parity-execution-evidence.md` Round 3, production-hardening in Round 4 — a second, deeper `execute_async` cancellation defect found and fixed structurally, not patched around; see Round 4 §9) · **Priority:** P1 · **Lane:** Execution Engine · **Dependencies:** P7, P8, P9, and **Gate G6 sign-off before implementation starts** · **Architecture deviation from this card's own original sketch, recorded explicitly (not silently substituted):** the maintainer's Gate G6 authorization (§7) specified a concrete public API — a new `libipynb.execution` package (`ExecutionOptions`/`ExecutionResult`/`NotebookExecutor` protocol) plus `libipynb.adapters.jupyter_execute.LocalJupyterExecutor` — superseding this card's own "TBD at implementation time" `execute_notebook(engine="kernel")` sketch. The subprocess engine (`adapters/execute.py`) is untouched, byte-for-byte, exactly as this card's CRITICAL SAFETY CONSTRAINT required; the new engine is a fully separate, additive class, not a parameter on the existing function. Isolation-field reuse (Required work item 4, below) does not apply under the new architecture — `LocalJupyterExecutor` documents its own, different isolation posture (no `isolate_cwd`/`isolate_env`-equivalent; kernel inherits the caller's environment/cwd by default, `extra_env`/`working_directory` are additive, not isolating) rather than reusing the subprocess engine's fields, since the two engines have genuinely different process-launch mechanisms (`jupyter_client`'s `KernelManager.start_kernel`, not a direct `subprocess.run` this module controls).
...
**Status:** `completed_verified` (2026-08-17, evidence in `full-parity-execution-evidence.md` Round 3) · **Priority:** P1 · **Lane:** Execution Engine · **Dependencies:** P4a-1 · **Mutate-vs-return decision (recorded per this card's own §"API implications" requirement):** neither, exclusively — `LocalJupyterExecutor.execute()`/`execute_async()` return a **new** `NotebookDocument` by default (`ExecutionOptions.in_place=False`, matching `merge_notebooks()`'s precedent), but also support `in_place=True` (mutating the caller's document and returning that same object, matching `cleanup()`'s precedent) as an explicit, tested opt-in. Justification: a non-mutating default is the safer choice for an execution API specifically (a caller who runs a notebook to inspect its output should not have silently mutated their own in-memory document unless they asked for it), while still offering the `cleanup()`-style in-place path for callers who want it — both existing precedents are honored as alternatives rather than forcing a single, less flexible choice.
...
**Status:** `completed_verified` (2026-08-17, evidence in `full-parity-execution-evidence.md` Round 3) · **Priority:** P2 · **Lane:** Execution Engine · **Dependencies:** P4a-1, P4b · **Shipped as:** `libipynb execute <source> [-o PATH] --acknowledge-unsandboxed [--kernel NAME] [--cell-timeout SECONDS] [--kernel-startup-timeout SECONDS] [--on-error stop|continue] [--no-interrupt-on-timeout] [--working-directory PATH] [--record-timing] [--force]` — no `--engine` flag (this card's original sketch predates the Gate G6-authorized architecture decision that the two engines are separate classes, not one function's parameter; the subprocess engine has no CLI path either, before or after this card, and remains out of scope here). `--force` (refuse to overwrite SOURCE via `-o` unless given) was added beyond this card's original scope, satisfying the mission brief's explicit "refuse accidental input overwrite" requirement.
...
**Superseded 2026-08-24 (LIBIPYNB-Q26, `plans/publication-readiness-plan-2026-08-24.md` Phase 2):** path (b)'s documented-decision outcome above no longer describes the current state -- `.github/workflows/ci.yml`'s `test` job now runs a real Windows and macOS leg (Python 3.12, the core `unit`/`integration`/`security` suite) on every push/PR, taking path (a) for GitHub Actions specifically, with real evidence recorded in that commit's message (an `act`-based local execution plus this taskcard's own reasoning about GitHub-hosted-runner semantics -- `act` cannot itself emulate non-Linux runners, so genuine Windows/macOS coverage is confirmed only once a real push triggers GitHub's actual hosted runners, not yet done in this engagement). The internal GitLab mirror's `.gitlab-ci.yml` remains Linux-only, unchanged -- this amendment does not retroactively invalidate this card's original 2026-08-13 closeout (accurate for GitLab CI, which is what existed then), it records that a second CI system was added later with broader platform coverage. README.md's "Platform support" section was updated in the same commit to match. Not re-opened as `not_attempted`: the documented-decision path this card actually took remains a true historical record of that specific decision at that specific time.
...
|---|---|---|---|---|
| 2026-08-17 | P4a-1, P4b, P4c | Babar Raza (repository maintainer — verified: `git config user.name`/`user.email` and `pyproject.toml`'s `authors` entry both match the requester of this session's mission) | **Approved** | Explicit, written, pre-implementation authorization to widen the execution surface with a real Jupyter-kernel-protocol backend, delivered as a full product specification for this exact capability (binding constraints: opt-in only, `exec`-extra-gated, no sandbox claim, trusted-local-execution posture, explicit security documentation). Superscedes the plan's original `engine="kernel"` sketch on `execute_notebook()` (P4a-1's own text called that naming "TBD at implementation time") with a concrete, maintainer-specified public API: a new `libipynb.execution` package (`ExecutionOptions`/`ExecutionResult`/`NotebookExecutor` protocol) plus `libipynb.adapters.jupyter_execute.LocalJupyterExecutor`, backed by `nbclient` (Option A from the mission's own feasibility comparison — see the implementation's module docstring for the Option A vs. B vs. C analysis). Resource-limit hardening (CPU/memory/disk/network caps) remains explicitly out of scope, matching P4a-1's original non-goals. This sign-off does not cover P4a-2 (multi-language widening) or P5a-c (papermill-style parameter injection) as dedicated taskcards — architecturally the new backend does not hardcode a Python-only refusal (unlike the subprocess engine), so multi-language execution is not blocked the way the subprocess engine's `_resolve_kernel` blocks it, but only Python/`ipykernel` is actually tested in this round (no R/Julia kernel is installed in this environment) — recorded as a documented limitation, not a completed P4a-2. |
| 2026-08-26 | LIBIPYNB-Q2b (wedged-kernel hard-kill escalation; tracked in `plans/state.json`'s `carried_forward_from_prior_plans`, not this plan's own §6 register) | Babar Raza (repository maintainer — verified: `git config user.name`/`user.email` match; sign-off given interactively via this session's `AskUserQuestion` gate, presented with the refined design and live-reproduction evidence before the decision) | **Approved as designed** | Explicit, pre-implementation authorization for a second-stage hard-kill escalation added to the existing per-cell watchdog (`adapters/jupyter_execute.py::_Tracker`): new opt-in `ExecutionOptions.hard_kill_grace_period: float \| None = None` (requires `cell_timeout` set and `interrupt_on_timeout=True`, validated in `__post_init__`) force-kills the kernel's OS process tree (reusing, not duplicating, `adapters/execute.py::_kill_process_tree`) if a cell is still running that many seconds after the watchdog's own fire point; new `ExecutionResult.hard_killed: bool`. Presented at decision time with concrete live-reproduction evidence (not source-reading alone): against a kernel configured to ignore SIGINT, `execute()` with only `cell_timeout=3`/`interrupt_on_timeout=True` did not return after 45s (15x its own budget, matching the predicted nbclient interrupt-retry-forever behavior); after directly killing the kernel's process tree mid-hang, the call surfaced a structured, non-crashing result 3.47s later. Same binding constraints as the P4a-1 sign-off above: opt-in only (`None` default, zero behavior change for existing callers), no change to the engine's "not sandboxed" documentation/framing, no new exception-handling machinery beyond what already exists. Does not cover per-message output-size capping, noted in `plans/state.json`'s own carried-forward entry as a separate, smaller, not-yet-designed follow-up deliberately not conflated with this fix. |
